...
class Controller(Updater): #ClientRequestListener
    ...
    def __init__(self, led_driver, midi, protocol = None, config = {}, inputs = [], ui = None, period_counter = None):
        ...
        if self._debug_stats:
            ...
            self._measurement_process_jitter = RuntimeMeasurement(get_option(config, "debugStatsInterval", update_interval))
            self._measurement_process_jitter.add_listener(self)
            self.add_updateable(self._measurement_process_jitter)            
    ...
    # Refreshes all action displays (LEDs and labels) immediately. Call after page change so the next page's colors/scheme load fully without waiting for the next update period.
    def refresh_action_displays(self):
        for input in self.inputs:
            for action in input.actions:
