...
class PagerAction(Callback, Action):
    ...
    def next_page(self):
        """Go to next page (for combo B+C)."""
        self._go_to_next_page()

    def prev_page(self):
        """Go to previous page (for combo A+B)."""
        self._go_to_prev_page()

    def _go_to_next_page(self):
        if not len(self.pages) or self.current_page_index < 0:
            return
        if self.__select_page_index is not None:
    ...
    def _apply_page_change(self):
        ...
        self.appl.refresh_action_displays()
