...
# TFT driver: 240x240 color display (ST7789; rowstart/colstart center image on panel)
...
# Fallback fonts if the requested one fails (e.g. corrupted/wrong format)
_FONT_FALLBACK_PATHS = ("/fonts/A12.pcf", "/fonts/PT40.pcf", "/fonts/H15.pcf")

# Buffered font loader
class AdafruitFontLoader:
    __fonts = {}

    # Returns a font (buffered). On ValueError (e.g. Unknown magic number), tries fallback fonts.
    def get(self, path):
        ...
        try:
            ...
            for fallback in _FONT_FALLBACK_PATHS:
                if fallback == path:
                    continue
                try:
                    font = _bitmap_font.load_font(fallback)
                    break
