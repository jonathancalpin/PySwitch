...
class Parser {
    ...
    #comboConfig = null;             // Cached combo config
    #pages = null;                   // Cached pages
     ...
     */
    async hasComboConfig() {
        await this.init();
        return this.#pySwitchParser.has_combo_config();
    }

    /**
     * Returns the combo configuration, or null if not using combo navigation
     */
    async getComboConfig() {
        if (this.#comboConfig) return this.#comboConfig;
        
        await this.init();
        const configJson = this.#pySwitchParser.combo_config();
        
        if (configJson) {
            this.#comboConfig = JSON.parse(configJson);
        }
    ...
    /**
     * Set the combo configuration
     */
    async setComboConfig(config) {
        await this.init();
        this.#pySwitchParser.set_combo_config(config);
        this.#comboConfig = config;
        this.updateConfig();
    }
    ...
    async getPages() {
        ...
        await this.init();
        const pagesJson = this.#pySwitchParser.pages();
    ...
    /**
     * Returns the ordered list of page variable names
     */
    async getPagesOrder() {
        await this.init();
        const orderJson = this.#pySwitchParser.pages_order();
        return orderJson ? JSON.parse(orderJson) : [];
    }

    /**
     * Update a specific page definition
     */
    async setPage(pageName, pageData) {
        await this.init();
        this.#pySwitchParser.set_page(pageName, pageData);
        this.#pages = null; // Clear cache
        this.updateConfig();
    }
     ...
     * Set the pages order
     */
    async setPagesOrder(pagesList) {
        await this.init();
        this.#pySwitchParser.set_pages_order(pagesList);
        this.updateConfig();
    }
     ...
     */
    async createPage(pageName, displayName, channel = 1) {
        const newPage = {
            name: displayName,
            channel: channel,
            midi_out: "USB",
        ...
        await this.setPage(pageName, newPage);
        
        // Add to pages order
        const order = await this.getPagesOrder();
        order.push(pageName);
        await this.setPagesOrder(order);
    ...
    async removePage(pageName) {
        // Remove from order
        const order = await this.getPagesOrder();
        const newOrder = order.filter(p => p !== pageName);
        await this.setPagesOrder(newOrder);
        
        // Note: The actual page definition remains in the file
        // but won't be used. Could add code to remove the assignment too.
        this.#pages = null;
    }

    /**
     * Generate a complete inputs.py for combo page navigation
     */
    async generateComboInputs(pagesData, comboConfig) {
        await this.init();
        return this.#pySwitchParser.generate_combo_inputs(
            JSON.stringify(pagesData),
            JSON.stringify(comboConfig)
        );
    }

    /**
     * Get a default combo config template
     */
    async getComboConfigTemplate() {
        await this.init();
        const template = this.#pySwitchParser.create_combo_config_template();
        return JSON.parse(template);
    }

    /**
     * Get default pages template
     */
    async getDefaultPagesTemplate() {
        await this.init();
        const template = this.#pySwitchParser.create_default_pages_template();
        return JSON.parse(template);
    }

    /**
     * Convert current configuration to combo navigation format.
     * This is useful when migrating from standard PySwitch to combo navigation.
     */
    async convertToComboNavigation() {
        await this.init();
        
        // Get current inputs
        const currentInputs = this.inputs();
        
        // Create default combo config
        const comboConfig = await this.getComboConfigTemplate();
        
        // Create a single page from current inputs
        const page = {
            var_name: 'PAGE_1',
            data: {
                name: 'Page 1',
                ...
                midi_out: 'USB',
                color_theme: [255, 255, 255],
                switches: currentInputs || []
            }
        };
        
        // Generate new inputs.py
        const newInputsPy = await this.generateComboInputs([page], comboConfig);
        
        // Update the configuration
        await this.updateFromSource('inputs_py', newInputsPy);
