...
class PySwitchRunner {
    ...
    static CACHE_VERSION = 3;   // Bump to bust cached Python/JSON modules
    ...
    async init(basePath = "") {
        ...
        if (!window.externalRefs) {
            window.externalRefs = {};
            window.externalRefs.protocolState = -10;  // No protocol until runner sets it
    ...
    async #loadModules(basePath) {
        ...
        return Promise.all([
            ...
            this.#loadModule("parser/ComboConfigExtractor.py", localPythonPath),
            this.#loadModule("parser/ComboCodeGenerator.py", localPythonPath),
            ...
            this.#loadModule("pyswitch/controller/controller_combo.py", circuitpyPath),
    ...
    getProtocolState() {
        ...
        const s = window.externalRefs.protocolState;
        return (s !== undefined && s !== null) ? s : -10;
    }
    ...
    async #loadModule(fileName, srcPath) {
        const url = (srcPath ? srcPath : "") + fileName;
        const bustUrl = url + (url.includes("?") ? "&" : "?") + "_v=" + PySwitchRunner.CACHE_VERSION;
        const code = await Tools.fetch(bustUrl);
