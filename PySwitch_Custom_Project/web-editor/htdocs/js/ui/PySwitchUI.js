...
class PySwitchUI {
    ...
    async build() {
        ...
        // Emulator display font scale (1 = device-like; <1 shrinks text so it fits 80x67 slots)
        if (typeof window !== 'undefined') {
            window.EMULATOR_FONT_SCALE = window.EMULATOR_FONT_SCALE ?? 0.5;
        }
        ...
        containerElement.append(
            this.container = $('<div class="container"/>').append(
                ...
                $('<div class="application"/>').append(
                    ...
                    // Client connection button (client-select so tests can find it before async inits; full state class set in ClientConnectionButton)
                    clientButtonElement = $('<div data-toggle="tooltip" class="client-select"/>'),

                    // Device area + live panel (CC / tap tempo) using the unused third of the screen
                    $('<div class="device-and-panel"/>').append(
                        $('<div class="device-area"/>').append(
                        ...
                        this.#livePanelElement = $('<div id="pyswitch-live-panel" class="pyswitch-live-panel"/>')
                    ),
                    ...
                    $('<div class="buttons" />').append(
                        ...
                        // Combo Page Navigation Editor
                        $('<div class="appl-button fas fa-layer-group" data-toggle="tooltip" title="Combo Page Navigation..."/>')
                        .on('click', async function() {
                            try {
                                await that.frontend.parserFrontend.showComboPageEditor();
        ...
        // Live panel (CC + tap tempo) next to device – create early so tests can find it before async inits
        if (this.#livePanelElement && this.#livePanelElement.length) {
            this.#livePanel = new LivePanel(this.#livePanelElement[0]);
        }
    ...
    async #initMidiMonitor() {
        ...
        // Forward MIDI to both MidiMonitor tab and live panel (CC + tap tempo)
        if (this.#livePanel) {
            const that = this;
            this.#controller.pyswitch.setMidiMonitor({
                monitorInput: function(message) {
                    that.midiMonitor.addMessage({ message: message, direction: "in" });
                    that.#livePanel.addMessage(message, "in");
                },
                monitorOutput: function(message) {
                    that.midiMonitor.addMessage({ message: message, direction: "out" });
                    that.#livePanel.addMessage(message, "out");
                }
            });
        }
    }
