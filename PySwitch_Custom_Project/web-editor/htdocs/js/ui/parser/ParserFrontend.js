...
class ParserFrontend {
    ...
    parser = null;
    icons = null;
    comboPanel = null;  // Combo page configuration panel

    #controller = null;
    #toUpdate = [];    // Queue for parser frontends which had changes recently.
    #comboPageProperties = null;
    ...
    /**
     * Shows the combo page configuration editor.
     */
    async showComboPageEditor() {
        const that = this;
        ...
        const browser = this.#controller.ui.getPopup({
            ...
            additionalClasses: "combo-page-editor-popup",
            wide: true
        });

        this.#comboPageProperties = new ComboPageProperties(
            this.parser,
            this.#controller,
            async function() {
                await that.updateConfig();
            }
        );

        const propsContent = await this.#comboPageProperties.create();
        if (!propsContent) return null;
    ...
    /**
     * Check if the current configuration uses combo page navigation
     */
    async hasComboNavigation() {
        return await this.parser.hasComboConfig();
    }
