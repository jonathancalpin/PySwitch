...
class TemplatesProvider extends BrowserProvider {
    ...
    async getToc(browser) {
        ...
        // Load TOC data (try PHP first for Docker/Apache, fall back to static JSON)
        let toc;
        try {
            toc = JSON.parse(await Tools.fetch(this.#path));
        } catch (_) {
            toc = JSON.parse(await Tools.fetch(this.#path.replace(/\.php$/, ".json")));
        }
