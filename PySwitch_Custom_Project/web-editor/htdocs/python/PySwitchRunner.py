...
class PySwitchRunner:
    ...
    def _init_default(self, display_width, display_height, client, config_py, comm_settings):
        with patch.dict(sys.modules, {
            ...
            # Detect ComboPager for combo paging support
            combo_pager = None
            try:
                from inputs import ComboPager
                combo_pager = ComboPager
            except ImportError:
                pass
            ...
            # Use ComboController when ComboPager is available for A+B/B+C combo paging
            if combo_pager:
                from pyswitch.controller.controller_combo import ComboController
                ControllerClass = ComboController
            else:
            ...
            if combo_pager:
                controller_kwargs["pager"] = combo_pager

            self.controller = ControllerClass(**controller_kwargs)
    ...
    def _init_explore_mode(self, display_width, display_height):
        with patch.dict(sys.modules, {
            ...
            # Set protocol state so the UI connection button is correct before first tick
            externalRefs.protocolState = self.protocol.state if self.protocol else -10
