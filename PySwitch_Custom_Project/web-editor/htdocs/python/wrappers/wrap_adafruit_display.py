...
def _get_emulator_font_scale():
    """Scale factor for font size in emulator (browser). Default 0.5 so text fits 80x67 slots; 1 only on device."""
    try:
        from js import window
        v = getattr(window, 'EMULATOR_FONT_SCALE', None)
        if v is not None and v != 0:
            return float(v)
        # In emulator but unset: use 0.5 so text doesn't overflow (wrapper only runs in browser)
        return 0.5
    except Exception:
        pass
    return 0.5
...
class WrapFont:
    ...
    def size(self, scale = 1):
        ...
        if not nums:
            return 20 * scale * _get_emulator_font_scale()
        return int(nums[0]) * scale * _get_emulator_font_scale()
