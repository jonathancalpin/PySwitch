...
# Modified process.py with combo detection and page switching support
#
# Compatible with PySwitch web interface (MidiBridge enabled)
#
#################################################################################################################################
...
if not _get_option(_Config, "exploreMode"):
    ...
    try:
        ...
        # Check if multi-page configuration exists
        _Pages = None
        _ComboConfig = None
        try:
            from inputs import Pages as _Pages
        except ImportError:
            pass
        
        try:
            from inputs import ComboConfig as _ComboConfig
        except ImportError:
            pass

        # Choose controller based on whether pages are defined
        if _Pages:
            # Use Multi-Page Controller with combo detection
            from pyswitch.controller.controller_multipage import MultiPageController as _Controller
            print("[PySwitch] Multi-page mode enabled")
            print(f"[PySwitch] Pages: {len(_Pages)}")
