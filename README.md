...
You can run the emulator locally without Docker or PHP. A build script packages everything into a self-contained directory that only needs Python 3 and a web browser.
...
Default output is `./portable-emulator/`. The script copies the web editor, firmware library, and examples, then generates static `toc.json` files so no server-side PHP is needed.

**Run:**

- **macOS:** Double-click `start-emulator.command` in the output folder
- **Any OS:** `cd portable-emulator && python3 -m http.server 8000`, then open http://localhost:8000

## Custom Firmware Features

The firmware in `content/` includes several features beyond basic switch actions. These are all configurable through the [Emulator](https://pyswitch.tunetown.de) graphically.

![Emulator with combo paging and display colors](docs/images/emulator-combo-paging.png)

### Combo Page Navigation

Switches can be organized into multiple pages. Pressing two adjacent switches simultaneously (within 50ms) changes pages:

- **A+B** = next page
- **B+C** = previous page

Each page has a name and color shown in the display footer. This lets you fit more functionality onto a 6-switch controller without extra hardware.

### Display Color Toggle

Actions with `toggle=True` show a bright color when active and dim when inactive. LED brightness is controlled via `led_brightness` and `led_brightness_off` parameters, giving clear visual feedback of on/off state.

### PC Preset Groups

The `group="name"` parameter creates radio-button behavior for Program Change presets: activating one preset automatically deactivates others in the same group. Use per-channel groups with `group=f"pc_ch{channel}"` to have independent radio groups for different MIDI channels.
