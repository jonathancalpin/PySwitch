...
class DisplayEditor {
    ...
    async init() {
        ...
        // Get raw splashes tree (deep copy). Use default when display_py has no Splashes assignment.
        const rawSplashes = this.getConfig().parser.splashes();
        if (!rawSplashes || typeof rawSplashes !== "object") {
            this.splashes = this.#createDefaultSplashes();
        } else {
            this.splashes = JSON.parse(JSON.stringify(rawSplashes));
        }
    ...
    /**
     * Build default Splashes when display_py has no Splashes assignment (avoids freeze/throw).
     */
    #createDefaultSplashes() {
        const definition = this.#getFirstCallbackDefinitionByTarget("Splashes");
        if (!definition || !definition.meta?.data?.splashRootParameter) {
            throw new Error("Display editor: no Splashes callback definition found.");
        }
        const paramName = definition.meta.data.splashRootParameter;
        const defaultRoot = {
            name: "DisplayElement",
            arguments: [
                {
        ...
        return {
            ...
            arguments: [
                { name: paramName, value: defaultRoot }
            ]
        };
    }

    /**
     * Returns the first callback definition with the given target (e.g. "Splashes").
     */
    #getFirstCallbackDefinitionByTarget(target) {
        for (const cl of this.availableCallbacks) {
            for (const cb of cl.callbacks) {
                if (cb.target === target) return cb;
            }
        }
        return null;
    }
