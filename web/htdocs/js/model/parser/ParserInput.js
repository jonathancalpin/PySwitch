class ParserInput extends ParserTreeElement {
    ...
    actions(hold = false) {
        ...
        const rawValue = actions.value;
        const ensureArray = (v) => {
            if (Array.isArray(v)) return v;
            if (!v || typeof v !== "object") return [];
            if (v.left !== undefined && v.right !== undefined)
                return [].concat(ensureArray(v.left), ensureArray(v.right));
            if (Object.keys(v).every((k) => /^\d+$/.test(k))) return Object.values(v);
            return Object.values(v);
        };
        const actionsArray = ensureArray(rawValue);
        const ret = actionsArray
